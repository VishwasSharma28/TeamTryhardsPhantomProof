        ...
        <div className="grid md:grid-cols-3 gap-5 max-w-6xl mx-auto">
          {services.map((service, i) => (
            <motion.div
              ...
              className={`group glass-panel p-8 lg:p-10 cursor-default transition-all duration-500 hover:shadow-2xl hover:border-cyan-400/20 ${service.glow}`}
            >
              {/* Icon */}
              <div
                className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${service.accent} flex items-center justify-center text-cyan-400 mb-6 group-hover:scale-110 transition-transform duration-500`}
              >
                {service.icon}
              </div>
              ...
              {/* Bottom accent */}
              <div className="mt-8 h-px bg-gradient-to-r from-white/[0.06] to-transparent group-hover:from-cyan-400/20 transition-all duration-500" />
            </motion.div>
          ))}
        </div>
      </div>
