  ...
  return (
    <section id="about" ref={ref} className="relative py-28 lg:py-36">
      ...
      >
        ...
        >
          Phantom AI is a next-generation verification platform combining 
          advanced machine learning, computer vision, and digital forensics to 
          analyze and authenticate digital media at scale.
        </motion.p>

        {/* Metrics Grid */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 lg:gap-6 max-w-5xl mx-auto items-stretch">
          {stats.map((stat, i) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 40 }}
            ...
            >
              <div className="text-3xl md:text-4xl lg:text-5xl font-black text-white mb-3 group-hover:drop-shadow-[0_0_15px_rgba(255,255,255,0.8)] transition-all duration-300 tracking-tighter">
                {stat.value}
              </div>
              <div className="text-[10px] md:text-xs text-slate-400 font-bold uppercase tracking-[0.2em] leading-tight">
                {stat.label}
              </div>
            </motion.div>
          ))}
